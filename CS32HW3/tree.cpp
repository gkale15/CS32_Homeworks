...
void separate(double a[], int n, double separator, int& firstNotGreater, int& firstLess)
  ...
  void order(double a[], int n)
  {
      ...
      int firstnotgrater;
      int firstless;
      separate(a, n, a[n/2], firstnotgrater, firstless);    //look at middle term in array and run separate ftom there
      order(a,firstnotgrater);   //order first half again
      order(a+firstless, n-firstless);
      //orer ladst half again
      //this repeats until the length of the sequence trying to be ordered is 1
  }
